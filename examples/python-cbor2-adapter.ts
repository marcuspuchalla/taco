/**
 * Python cbor2 Adapter for CBOR Test Suite
 * Implements the CBOREncoder interface using Python's cbor2 library via bridge script
 */
...
/**
 * Adapter for Python cbor2 library
 */
export class PythonCbor2Adapter implements CBOREncoder {
  name = 'python-cbor2';
  version = '5.7.1';
  features = ['bigint', 'maps', 'typed-arrays', 'dates', 'tags', 'python'];
  ...
  /**
   * Encode value to CBOR bytes using Python cbor2
   */
  encode(value: unknown): Uint8Array {
    try {
      const jsonInput = JSON.stringify(value);
      const command = `echo '${jsonInput}' | ${this.pythonPath} ${this.bridgeScript} cbor2 encode`;
      ...
      throw new Error(`python-cbor2 encode error: ${(error as Error).message}`);
    }
  }

  /**
   * Decode CBOR bytes to value using Python cbor2
   */
  decode(bytes: Uint8Array): unknown {
    try {
      ...
      const command = `echo '${hex}' | ${this.pythonPath} ${this.bridgeScript} cbor2 decode`;
      ...
      throw new Error(`python-cbor2 decode error: ${(error as Error).message}`);
    }
  }
...
if (require.main === module) {
  ...
  console.log('Testing Python cbor2 adapter...\n');

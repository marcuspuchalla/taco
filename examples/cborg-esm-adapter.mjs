/**
 * cborg ESM Adapter for CBOR Test Suite
...
/**
 * Adapter for cborg library (ESM version)
 */
export class CborgAdapter {
  name = 'cborg';
  version = '4.2.18';
  ...
  encode(value) {
    try {
      // cborg encode returns Uint8Array
      return encode(value);
    } catch (error) {
      throw new Error(`cborg encode error: ${error.message}`);
    }
  }
  ...
  decode(bytes) {
    try {
      ...
      throw new Error(`cborg decode error: ${error.message}`);
    }
  }
  ...
  /**
   * cborg encodes deterministically by default
   */
...
console.log('Testing cborg ESM adapter...\n');

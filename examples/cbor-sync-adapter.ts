/**
 * cbor-sync Adapter for CBOR Test Suite
 * Implements the CBOREncoder interface for the cbor-sync library
 */
...
/**
 * Adapter for cbor-sync library
 */
export class CborSyncAdapter implements CBOREncoder {
  name = 'cbor-sync';
  version = '1.0.4';
  features = ['bigint', 'maps', 'typed-arrays', 'dates', 'tags'];
  ...
  encode(value: unknown): Uint8Array {
    try {
      ...
      throw new Error(`cbor-sync encode error: ${(error as Error).message}`);
    }
  }
  ...
  decode(bytes: Uint8Array): unknown {
    try {
      ...
      throw new Error(`cbor-sync decode error: ${(error as Error).message}`);
    }
  }

  /**
   * Convert cbor-sync types to expected test format
   */
  private convertTypes(value: any): any {
    ...
    // Handle tagged values - cbor-sync may return objects with Symbol properties
    // Check for common tag patterns
    if (value && typeof value === 'object') {
      // Check if it's a tagged value by looking for tag property
      const symbols = Object.getOwnPropertySymbols(value);
      for (const sym of symbols) {
        const symString = sym.toString();
        if (symString.includes('tag')) {
          const tagValue = value[sym];
          if (typeof tagValue === 'number') {
            return this.convertTag(tagValue, this.convertTypes(value.value || value));
          }
        }
      }
    }
...
if (require.main === module) {
  ...
  console.log('Testing cbor-sync adapter...\n');
  ...
  // Test 4: Array
  console.log('\nTest 4: Encode/decode array [1, 2, 3]');
  const bytes4 = adapter.encode([1, 2, 3]);
  ...
  console.log('  Success:', JSON.stringify(decoded4) === '[1,2,3]');

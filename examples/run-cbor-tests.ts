/**
 * Run CBOR Test Suite against cbor (node-cbor) library
 */
...
async function main() {
  console.log('═══════════════════════════════════════════════════════');
  console.log('  CBOR Test Suite - Testing cbor (node-cbor)');
  console.log('═══════════════════════════════════════════════════════\n');
  ...
  const runner = new CBORTestRunner(adapter, {
    verbose: false,
    testRoundTrip: true,
    testCanonical: true,  // node-cbor supports canonical encoding
    failFast: false,
    timeout: 5000
  });
  ...
  await reporter.save(results, 'json', path.join(reportsDir, 'cbor-results.json'));
  console.log('✓ Saved JSON report: reports/cbor-results.json');

  await reporter.save(results, 'html', path.join(reportsDir, 'cbor-results.html'));
  console.log('✓ Saved HTML report: reports/cbor-results.html');

  await reporter.save(results, 'markdown', path.join(reportsDir, 'cbor-results.md'));
  console.log('✓ Saved Markdown report: reports/cbor-results.md');

  await reporter.save(results, 'csv', path.join(reportsDir, 'cbor-results.csv'));
  console.log('✓ Saved CSV report: reports/cbor-results.csv');

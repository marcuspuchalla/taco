/**
 * CBOR Test Container - Node.js with dag-cbor library (ESM)
 */
...
const PORT = 8080;
const LIBRARY_NAME = 'dag-cbor';
const LIBRARY_VERSION = '9.2.5';
...
function toJsonSafe(value) {
  ...
  // Handle CID objects from dag-cbor
  if (value && typeof value === 'object' && value.toString && value['/']) {
    return { __cid__: value.toString() };
  }
...
function decodeHex(hexString) {
  ...
  try {
    ...
    const result = dagCbor.decode(bytes);
...
function encodeValue(value) {
  ...
  try {
    ...
    const encoded = dagCbor.encode(jsValue);

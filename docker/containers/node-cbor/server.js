/**
 * CBOR Test Container - Node.js with node-cbor library
 */
...
const PORT = 8080;
const LIBRARY_NAME = 'cbor';
const LIBRARY_VERSION = '10.0.11';
...
function fromJsonSafe(value) {
  ...
  if (typeof value === 'object' && !Array.isArray(value)) {
    if ('__cbor_bytes__' in value) return Buffer.from(value.__cbor_bytes__, 'hex');
    ...
    if ('__cbor_tag__' in value && '__cbor_value__' in value) {
      return new cbor.Tagged(value.__cbor_tag__, fromJsonSafe(value.__cbor_value__));
    }
...
function decodeHex(hexString) {
  ...
  try {
    ...
    const result = cbor.decodeFirstSync(bytes);
...
function encodeValue(value) {
  ...
  try {
    ...
    const encoded = cbor.encodeOne(jsValue);
